...
if(isset($data)){
	...
	if ($is_token_expired || !$is_signature_valid) {
        ...
        $query = "SELECT COUNT(*) AS order_num FROM orders";
        $result = mysqli_query($conn, $query);

        if ($result) {
            $order_num = mysqli_fetch_assoc($result)['order_num'];
            $response = array('success' => false, 'message' => 'got the number of orders', 'data' => $order_num);
            echo json_encode($response);
        }else{
