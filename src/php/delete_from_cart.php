...
if(isset($data)){
	...
	if ($is_token_expired || !$is_signature_valid) {
        ...
        if (mysqli_num_rows($result) == 0) {
            $response = ['success' => false, 'message' => 'there is no such a product in your cart.', 'reason' => 'already have'];
            echo json_encode($response);
            exit;
        }else{
            ...
            if(mysqli_query($conn, $sql)){
                $response = ['success' => true, 'message' => 'product Deleted successfuly from the cart!'];
                echo json_encode($response);
            }else{
