...
if(isset($data)){
	...
	if ($is_token_expired || !$is_signature_valid) {
        ...
        if (mysqli_num_rows($result) == 1) {
            $response = ['success' => false, 'message' => 'Product already exists in the cart.', 'reason' => 'already have'];
            echo json_encode($response);
            exit;
        }else{
            ...
            if(mysqli_query($conn, $sql)){
                $response = ['success' => true, 'message' => 'Product Added successfuly to the cart!'];
                echo json_encode($response);
            }else{
